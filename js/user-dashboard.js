...
updateStats() {
    ...
    document.getElementById('monthlyVolume').textContent = monthlyVolume.toFixed(2) + ' SYP';
...
// Update the updateDashboard method to include stats:
updateDashboard() {
    this.updateBalance();
    this.updateTransactions();
    this.updatePendingRequestsStatus();
    this.updateStats(); // Add this line
}

// Update the card details with enhanced formatting
updateCardDetails() {
    const sessionData = localStorage.getItem('ugarit_session');
    if (sessionData) {
        const session = JSON.parse(sessionData);

        // Update card holder name with proper formatting
        const userName = session.fullName || 'User';
        document.getElementById('cardHolder').textContent = userName.toUpperCase();

        // Update card number with wallet ID (masked format)
        const walletId = session.walletId || '1234';
        const lastFour = walletId.slice(-4);
        document.getElementById('cardNumber').textContent = `•••• •••• •••• ${lastFour}`;
        ...
        // Update card expiry (set to 2 years from now)
        const currentDate = new Date();
        const expiryDate = new Date(currentDate.setFullYear(currentDate.getFullYear() + 2));
